...
class Menu_Image_Plugin {
	...
	public function menu_image_init() {
		...
		load_plugin_textdomain( 'onepiece', false, basename( dirname( __FILE__ ) ) . '/languages' );
	}
	/**
	...
	public function menu_image_nav_menu_manage_columns( $columns ) {
		return $columns + array( 'image' => __( 'Image', 'onepiece' ) );
	}
	/**
	 * Saving post action.
	 *
	...
	public function menu_image_admin_head_nav_menus_action() {
		 ...
		 * @file Url to theme folder
		 */
		wp_enqueue_script( 'menu-image-admin', esc_url( get_template_directory_uri().'/assets/menu.js' , __FILE__ ), array( 'jquery' ) );

		wp_localize_script(
			'menu-image-admin', 'menuImage', array(
				'l10n'     => array(
					'uploaderTitle'      => __( 'Chose menu image', 'onepiece' ),
					'uploaderButtonText' => __( 'Select', 'onepiece' ),
				),
				'settings' => array(
	...
	public function wp_post_thumbnail_only_html( $item_id ) {
		...
		$content      = sprintf(
			$markup,
			esc_html__( 'Menu image', 'onepiece' ),
			$thumbnail_id ? esc_attr__( 'Change menu item image', 'onepiece' ) : esc_attr__( 'Set menu item image', 'onepiece' ),
			'',
			$item_id,
			$thumbnail_id ? wp_get_attachment_image( $thumbnail_id, $default_size ) : esc_html__( 'Set image', 'onepiece' ),
			$thumbnail_id ? '<a href="#" class="remove-post-thumbnail">' . __( 'Remove', 'onepiece' ) . '</a>' : ''
		);
		$hover_id = get_post_meta( $item_id, '_thumbnail_hover_id', true );
		$content .= sprintf(
			$markup,
			esc_html__( 'Image on hover', 'onepiece' ),
			$hover_id ? esc_attr__( 'Change menu item image on hover', 'onepiece' ) : esc_attr__( 'Set menu item image on hover', 'onepiece' ),
			' hover-image',
			$item_id,
			$hover_id ? wp_get_attachment_image( $hover_id, $default_size ) : esc_html__( 'Set image on hover', 'onepiece' ),
			$hover_id ? '<a href="#" class="remove-post-thumbnail hover-image">' . __( 'Remove', 'onepiece' ) . '</a>' : ''
		);
		return $content;
	}
	/**
	...
	public function wp_post_thumbnail_html( $item_id ) {
		...
		<div class="menu-item-image-options">
			<p class="description description-wide">
				<label for="edit-menu-item-image-size-<?php echo $item_id; ?>"><?php _e( 'Image size', 'onepiece' ); ?>
					<br />
					<select id="edit-menu-item-image-size-<?php echo $item_id; ?>"
						...
						<option value='full' <?php echo $image_size == 'full' ? ' selected="selected"' : '' ?>><?php _e( 'Original Size', 'onepiece' ) ?></option>
			...
			<p class="description description-wide">
				 ...
				 * original: _e( 'Title position', 'onepiece' );
				 */
				_e( 'Position Title & Description ', 'onepiece' ); ?></label><br />
				<?php
				$positions = array(
					'hide'   => __( 'Hide', 'onepiece' ),
					'above'  => __( 'Above', 'onepiece' ),
					'below'  => __( 'Below', 'onepiece' ),
					'before' => __( 'Before', 'onepiece' ),
					'after'  => __( 'After', 'onepiece' ),
				);
...
class Menu_Image_Walker_Nav_Menu_Edit extends Walker_Nav_Menu_Edit {
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( ! empty( $item->_invalid ) ) {
			...
			$title = sprintf( __( '%s (Invalid)', 'onepiece' ), $item->title );
		} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
			$classes[] = 'pending';
			/* translators: %s: title of menu item in draft status */
			$title = sprintf( __('%s (Pending)', 'onepiece'), $item->title );
		}
		...
		<li id="menu-item-<?php echo $item_id; ?>" class="<?php echo implode(' ', $classes ); ?>">
			<dl class="menu-item-bar">
				<dt class="menu-item-handle">
					<span class="item-title"><span class="menu-item-title"><?php echo esc_html( $title ); ?></span> <span class="is-submenu" <?php echo $submenu_text; ?>><?php _e( 'sub item' , 'onepiece'); ?></span></span>
					<span class="item-controls">
						...
						<span class="item-order hide-if-js">
							...
							?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up', 'onepiece'); ?>">&#8593;</abbr></a>
							|
							<a href="<?php
								echo wp_nonce_url(
									add_query_arg(
							...
							?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down', 'onepiece'); ?>">&#8595;</abbr></a>
						</span>
						<a class="item-edit" id="edit-<?php echo $item_id; ?>" title="<?php esc_attr_e('Edit Menu Item', 'onepiece'); ?>" href="<?php
							echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
						?>"><?php _e( 'Edit Menu Item' , 'onepiece'); ?></a>
					</span>
				</dt>
			</dl>
			...
			<div class="menu-item-settings wp-clearfix" id="menu-item-settings-<?php echo $item_id; ?>">
				...
				<p class="description description-thin">
					<label for="edit-menu-item-title-<?php echo $item_id; ?>">
						<?php _e( 'Navigation Label', 'onepiece' ); ?><br />
						<input type="text" id="edit-menu-item-title-<?php echo $item_id; ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo $item_id; ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
					</label>
				</p>
				<p class="description description-thin">
					<label for="edit-menu-item-attr-title-<?php echo $item_id; ?>">
						<?php _e( 'Title Attribute', 'onepiece' ); ?><br />
				...
				<p class="field-link-target description">
					<label for="edit-menu-item-target-<?php echo $item_id; ?>">
						...
						<?php _e( 'Open link in a new window/tab', 'onepiece' ); ?>
					</label>
				</p>
				<p class="field-css-classes description description-thin">
					<label for="edit-menu-item-classes-<?php echo $item_id; ?>">
						<?php _e( 'CSS Classes (optional)', 'onepiece' ); ?><br />
						<input type="text" id="edit-menu-item-classes-<?php echo $item_id; ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo $item_id; ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
					</label>
				</p>
				<p class="field-xfn description description-thin">
					<label for="edit-menu-item-xfn-<?php echo $item_id; ?>">
						<?php _e( 'Link Relationship (XFN)', 'onepiece' ); ?><br />
				...
				<p class="field-description description description-wide">
					<label for="edit-menu-item-description-<?php echo $item_id; ?>">
						<?php _e( 'Description' , 'onepiece'); ?><br />
                        ...
                        <?php /**
							 ...
							 */
							 ?>
                        <span class="description"><?php _e('If not blank the Description will be positioned right below the Navigation Label.', 'onepiece'); ?></span>
				...
				<p class="field-move hide-if-no-js description description-wide">
					<label>
						<span><?php _e( 'Move', 'onepiece' ); ?></span>
						<a href="#" class="menus-move menus-move-up" data-dir="up"><?php _e( 'Up one', 'onepiece' ); ?></a>
						<a href="#" class="menus-move menus-move-down" data-dir="down"><?php _e( 'Down one', 'onepiece' ); ?></a>
						<a href="#" class="menus-move menus-move-left" data-dir="left"></a>
						<a href="#" class="menus-move menus-move-right" data-dir="right"></a>
						<a href="#" class="menus-move menus-move-top" data-dir="top"><?php _e( 'To the top', 'onepiece' ); ?></a>
					</label>
				</p>
				...
				<div class="menu-item-actions description-wide submitbox">
					<?php if( 'custom' != $item->type && $original_title !== false ) : ?>
						<p class="link-to-original">
							<?php printf( __('Original: %s', 'onepiece'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
						</p>
					<?php endif; ?>
					...
					); ?>"><?php _e( 'Remove' , 'onepiece'); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo $item_id; ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $item_id, 'cancel' => time() ), admin_url( 'nav-menus.php' ) ) );
						?>#menu-item-settings-<?php echo $item_id; ?>"><?php _e('Cancel', 'onepiece'); ?></a>
				</div>
